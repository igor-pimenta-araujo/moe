<div style="min-height: calc(100vh - 134px)" >
<div class="section no-pad-bot" id="index-banner">
    <div class="container">
        <br><br>
        <h1 class="header center green-text">Bem vindo! <?= $employer->employer_contact_name;?></h1>
        <div class="row center">
            <h5 class="header col s12 light">Aqui você terá acesso as suas funcionalidades de Empresa</h5>
        </div>
        <div class="row center">
            <a href="" id="download-button" class="btn-large waves-effect waves-light green darken-4">Vamos lá!</a>
        </div>
        <br><br>

    </div>
</div>


<div class="container">
    <div class="section">

        <!--   Icon Section   -->
        <div class="row">
            <div class="col s12 m4">
                <div class="icon-block">
                    <h2 class="center green-text"><i class="material-icons">flash_on</i></h2>
                    <h5 class="center">Velocidade</h5>

                    <p class="light">Com tantas opções você irá encontrar o estagiário que precisa.</p>
                </div>
            </div>

            <div class="col s12 m4">
                <div class="icon-block">
                    <h2 class="center green-text"><i class="material-icons">group</i></h2>
                    <h5 class="center">Experiência</h5>

                    <p class="light">Consultar/editar oportunidade de estágio, consultar estagiários interessados na empresa.</p>
                </div>
            </div>

            <div class="col s12 m4">
                <div class="icon-block">
                    <h2 class="center green-text"><i class="material-icons">settings</i></h2>
                    <h5 class="center">Funcionalidades</h5>

                    <p class="light">Cadastrar oportunidade de estágio, atualizar seus dados cadastrais.</p>
                </div>
            </div>
        </div>

    </div>
    <br><br>
</div>
</div>